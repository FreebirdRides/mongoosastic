...
function Mongoosastic (schema, pluginOpts) {
  ...
  function postSave (doc) {
    console.log('were in postsave')
    let _doc

    console.log('whats up doc', JSON.stringify(doc))

    function onIndex (err, res) {
      console.log('were in postsave:onIndex')
      if (!filter || !filter(doc)) {
        console.log('were in es-indexed')
        doc.emit('es-indexed', err, res)
      } else {
        console.log('were in es-filtered')
        doc.emit('es-filtered', err, res)
      }
    }

    if (doc) {
      console.log('were in postsave:onIndex if doc')
      _doc = new doc.constructor(doc)
      if (populate && populate.length) {
        populate.forEach(populateOpts => {
          console.log('were in _doc.populate(populateOpts)')
          _doc.populate(populateOpts)
        })
        _doc.execPopulate().then(popDoc => {
          console.log('were in popDoc.index(onIndex)')
          popDoc.index(onIndex)
        }).catch(onIndex)
      } else {
        console.log('were in _doc.index(onIndex)')
        _doc.index(onIndex)
      }
    }
  }
  ...
  function setUpMiddlewareHooks (inSchema) {
    ...
    console.log('we got into the hook setup')
    ...
    inSchema.post('findByIdAndUpdate', postSave)
